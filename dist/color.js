...
exports.ColorNames = exports.Color = void 0;
...
class Color {
    ...
    name() {
        ...
        return Object.keys(exports.ColorNames).find((x) => this.value === exports.ColorNames[x]);
    }
    static parse(value) {
        ...
        if (value.toLowerCase() in exports.ColorNames) {
            return exports.ColorNames[value.toLowerCase()];
        }
        let out = null;
        ...
        if ((out = this.hsl_regex.exec(value))) {
            const [, a, b, c, d, e] = out;
            const type = this.hue_from_type(Number(a), b);
        ...
        if ((out = this.rgb_regex.exec(value))) {
            ...
            return this.from_rgba({
                r: (0, util_1.clamp)(0, 0xff, Number(r)),
                g: (0, util_1.clamp)(0, 0xff, Number(g)),
                b: (0, util_1.clamp)(0, 0xff, Number(b)),
        ...
        if ((out = this.rgb_percentage_regex.exec(value))) {
            ...
            const p = 0xff / 100;
            return this.from_rgba({
                r: (0, util_1.clamp)(0, 255, Number(r) * p),
                g: (0, util_1.clamp)(0, 255, Number(g) * p),
                b: (0, util_1.clamp)(0, 255, Number(b) * p),
                a: a ? this.parse_alpha(a) : 0xff,
            });
        }
        throw new Error(`Unable to parse '${value}' as a color`);
    }
    static is_rgb_object(value) {
        return (0, util_1.is_object)(value, { r: "number", g: "number", b: "number" });
    }
    static is_rgba_object(value) {
        return this.is_rgb_object(value) && (0, util_1.is_object)(value, { a: "number" });
    }
    static is_hsl_object(value) {
        return (0, util_1.is_object)(value, { h: "number", s: "number", l: "number" });
    }
    static is_hsla_object(value) {
        return this.is_rgb_object(value) && (0, util_1.is_object)(value, { a: "number" });
    }
    static from_rgba(value) {
        if (!this.is_rgba_object(value)) {
            value = { a: 0xff, ...value };
        }
        const { r: r, g: g, b: b, a: a } = value;
        return (((r & 0xff) << 24) | ((g & 0xff) << 0x10) | ((b & 0xff) << 8) | (a & 0xff));
    }
    static from_hsla(value) {
        if (!this.is_hsla_object(value)) {
            value = { a: 1, ...value };
        }
        value.s = (0, util_1.clamp)(0, 1, value.s);
        value.a = (0, util_1.clamp)(0, 1, value.a);
        if (value.s === 0) {
            return this.from_rgba({
                r: 0xff,
                g: 0xff,
        ...
        value.h %= 1;
        value.l = (0, util_1.clamp)(0, 1, value.l);
        const q = value.l < 1 / 2
    ...
    static parse_alpha(a) {
        return (0, util_1.clamp)(0, 0xff, "%" === a[a.length - 1] ? (0xff / 100) * parseFloat(a) : +a * 0xff);
    }
    static short_hex_regex = /^#?([\da-f]{3,4})$/;
    static long_hex_regex = /^#?((?:[\da-f]{2}){3,4})$/;
    static rgb_regex = /^rgba?\((?<r>(?:\d*\.)?\d+)(?: +| *, *)(?<g>(?:\d*\.)?\d+)(?: +| *, *)(?<b>(?:\d*\.)?\d+)(?:(?: +| *, *)(?<a>\d+|\d*\.\d+|\d+(?:\.\d+)?%))?\)$/;
    static rgb_percentage_regex = /^rgba?\((?<r>(?:\d*\.)?\d+)%(?: +| *, *)(?<g>(?:\d*\.)?\d+)%(?: +| *, *)(?<b>(?:\d*\.)?\d+)%(?:(?: +| *, *)(?<a>\d+|\d*\.\d+|\d+(?:\.\d+)?%))?\)$/;
    static hsl_regex = /^hsla?\((?<h>(?:\d*\.)?\d+)(?<t>|deg|rad|grad|turn)(?: +| *, *)(?<s>(?:\d*\.)?\d+)%(?: +| *, *)(?<l>(?:\d*\.)?\d+)%(?:(?: +| *, *)(?<a>\d+|\d*\.\d+|\d+(?:\.\d+)?%))?\)$/;
}
exports.Color = Color;
exports.ColorNames = {
